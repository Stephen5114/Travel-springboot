<template>
  <div>
      ...
      <el-table :data="data.tableData" style="width: 100%" :header-cell-style="{ color: '#333', backgroundColor: '#eaf4ff' }">
        <el-table-column label="攻略主图" >
          <template #default="scope">
            <el-image :src="scope.row.img" :preview-src-list="[scope.row.img]" :preview-teleported="true" style="width: 40px; height: 40px; border-radius: 5px; display: block" />
          </template>
        </el-table-column>
    ...
    <el-dialog title="攻略信息" v-model="data.formVisible" width="70%" destroy-on-close>
      <el-form ref="formRef" :model="data.form" label-width="80px" style="padding: 20px 30px 10px 0">
        <el-form-item prop="img" label="攻略主图">
          <el-upload
              ...
              list-type="picture"
          >
            <el-button type="primary">Upload Img</el-button>
          </el-upload>
        </el-form-item>
    ...
    <el-dialog title="攻略信息" v-model="data.viewVisible" width="70%" destroy-on-close>
...
const handleAdd = () => {
  ...
  valueHtml.value = ''
  if (editorRef.value) {
    editorRef.value.setHtml('') // ✅ 强制清空编辑器内容
  }
}
...
const add = () => {
  console.log('aaaaaaaaa:'+data.form)
